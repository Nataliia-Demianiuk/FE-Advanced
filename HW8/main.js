class Student {
  ...
  getAverageMark(){
      if(this.allMarks.length > 0){
          return (this.allMarks.reduce((sum, mark) => sum += mark, 0) / this.allMarks.length).toFixed();
      } else {
          return null;
      }
  };
...
const student = new Student('МНТУ', '1', 'Віталій');
...
const budgetStudent = new BudgetStudent('МАУП', '2', 'Наталія');
